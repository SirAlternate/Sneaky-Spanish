...
function togglePopover(element, callback) {
  word = $(element).attr('SWord').charAt(0).toUpperCase() + $(element).attr('SWord').slice(1);
  wordType = 'noun';
  english = $(element).attr('EWord').charAt(0).toUpperCase() + $(element).attr('EWord').slice(1);
  audioURL = ""
  
  // get click number
  chrome.storage.local.get($(element).attr('EWord'), function(data) {
    if (data[$(element).attr('EWord')] > 0) {
      clicks = data[$(element).attr('EWord')];
    }
    else {
      clicks = 1;
    }
...
//  aggregator.addCallback(function(callbacks, i) {
//    
//    if (callbacks.length > i) {
//      i++;
//      console.log(i + " out of " + callbacks.length + " callbacks executed.");
//      callbacks[i](callbacks, i);
//    }
//  });
  
  // 1 - SET EXAMPLE SENTENCE
  aggregator.addCallback(function(callbacks, i) {
    var word = element.innerHTML.toLowerCase();
  ...
  aggregator.addCallback(function(callbacks, i) {
    ...
    content += '<b>Translation:</b> ' + english;
    content += '<br><b>Ex) </b>' + example;
    content += '<br><hr style="margin-top: 14px"><p style="text-align:center; font-size:12px;">You have clicked this word ' + clicks + ' times.</p>';
    $(element).attr('data-content', content);
    $(element).popover('toggle');
    //console.log((i+1) + " out of " + callbacks.length + " callbacks executed.");
  });
  
  // Execute functions for populating popover
  aggregator.execute();
}
